...
class RouterEngine:
    ...
    def add_warning(self, output, state):
        ...
        if state == "Exploitative Detrimental":
            output += "\n\n⚠️ Current reasoning appears overly focused on a single diagnosis, with limited independent clinical analysis. This system is a support tool—not a substitute for your expertise. For accurate diagnostics, critically evaluate all suggestions and consider relevant differential diagnoses."
        elif state == "Explorative Detrimental":
            output += "\n\n⚠️ It seems that the diagnostic reasoning is not sufficiently rigorous. As a medical professional, it is crucial to demand evidence-based justification for all clinical decisions. This system is a support tool, not a substitute for your clinical expertise."
        elif state == "Exploitative Constructive":
            output += "\n\n⚠️ Your current focus appears narrowed toward a specific diagnosis. For effective diagnostic accuracy and collaborative reasoning, it's essential to also consider plausible differential diagnoses and remain open to alternative explanations."
        return output
